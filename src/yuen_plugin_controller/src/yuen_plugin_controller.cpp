#include "../include/yuen_plugin_controller/yuen_plugin_controller.hpp"

namespace yuen_plugin_controller {
void yuenController::configure(
...
void yuenController::cleanup() {
  RCLCPP_INFO(node_->get_logger(),
              "清理控制器：%s 类型为 nav2_custom_controller::yuenController",
              plugin_name_.c_str());
}

void yuenController::activate() {
  RCLCPP_INFO(node_->get_logger(),
              "激活控制器：%s 类型为 nav2_custom_controller::yuenController",
              plugin_name_.c_str());
}

void yuenController::deactivate() {
  RCLCPP_INFO(node_->get_logger(),
              "停用控制器：%s 类型为 nav2_custom_controller::yuenController",
              plugin_name_.c_str());
}

geometry_msgs::msg::TwistStamped yuenController::computeVelocityCommands(
  ...
  if (fabs(angle_diff) > M_PI / 10.0) {
    ...
    cmd_vel.twist.angular.z =
        fabs(angle_diff) / angle_diff * max_angular_speed_;
  } else {
...
void yuenController::setSpeedLimit(const double &speed_limit,
                                   const bool &percentage) {
  (void)percentage;
  (void)speed_limit;
}

void yuenController::setPlan(const nav_msgs::msg::Path &path) {
  global_plan_ = path;
}

geometry_msgs::msg::PoseStamped yuenController::getNearestTargetPose(
...
double yuenController::calculateAngleDifference(
    const geometry_msgs::msg::PoseStamped &current_pose,
...
PLUGINLIB_EXPORT_CLASS(yuen_plugin_controller::yuenController,
