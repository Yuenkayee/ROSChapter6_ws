...
# 添加服务接口
from patrol_interfaces.srv import SpeachText
from sensor_msgs.msg import Image
from cv_bridge import CvBridge
import cv2

# 注意，BasicNavigator 在创建时就已经完成了几乎所有的 spin 动作并且在自动运行节点，因此不在主函数中再用 spin 运行节点
class PatrolNode(BasicNavigator):
    def __init__(self, node_name='patrol_node'):
        super().__init__(node_name)
        # 导航相关定义(声明相关参数,并将相关声明赋给变量)
    ...
    def get_pose_by_xyyaw(self, x, y, yaw):
        ...
        # 下面函数通过返回一个数组来表示四元数，按照顺序其含义分别是 x,y,z,w
