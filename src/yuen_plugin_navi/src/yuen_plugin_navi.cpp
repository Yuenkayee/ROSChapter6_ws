...
#include "yuen_plugin_navi/yuen_plugin_navi.hpp"

namespace yuen_plugin_navi {

void yuenPlanner::configure(
...
yuenPlanner::createPlan(const geometry_msgs::msg::PoseStamped &start,
  ...
  for (geometry_msgs::msg::PoseStamped pose : global_path.poses) {
    ...
    if (costmap_->worldToMap(pose.pose.position.x, pose.pose.position.y, mx,
                             my)) {
...
PLUGINLIB_EXPORT_CLASS(yuen_plugin_navi::yuenPlanner, nav2_core::GlobalPlanner)